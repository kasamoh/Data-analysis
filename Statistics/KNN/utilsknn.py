...
def frontiere_new_bundle(f, X, y, w=None, step=50, alpha_choice=1, colorbar=True,
                  samples=True,axe=None):
    """ trace la frontiere pour la fonction de decision f"""
    # construct cmap
...
class ErrorCurve(object):
    ...
    def fit_curve(self, X, y, Xtest, ytest):
        ...
        self.y = y
    def getk (self) :
        return self.k_range , self.errors
    ...
    def plotax(self, marker='o', maketitle=True,ax=None,**kwargs):
        ax.plot(self.k_range, self.errors, marker=marker, **kwargs)
        ax.set_xticklabels("K")
        ax.set_yticklabels("Test error")
        if maketitle:
            ax.set_yticklabels("number of training points : %d" % len(self.y))
...
class LOOCurve(object):
    ...
    def fit_curve(self, X, y, n_iter=200, test_size_fraction=0.1,random_state=1):
        def score_func(k):
            n_samples = len(X)
            ...
            loo = model_selection.ShuffleSplit(n_iter,
                                                ...
                                                train_size=n_samples-int(test_size_fraction*n_samples),
    ...
    def plot(self, marker='o', maketitle=True, **kwargs):
        ...
        best_k=(self.k_range[np.argmax(self.cv_scores)])
        plt.xlabel("K")
        plt.ylabel("Leave One Out Score (1-error rate)")
        if maketitle:
            plt.title("number of training points : %d , Meilleur valeur de k= %s" % (len(self.y) - 1,best_k))
